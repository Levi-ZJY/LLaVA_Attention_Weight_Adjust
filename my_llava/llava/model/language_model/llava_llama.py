...
class My_LlamaAttention_SDPA(LlamaSdpaAttention):
    ...
    def forward(
        ...
        use_cache: bool = False,
        Now_img_txt_token_spans = None,
        Attention_Change_Mod = None
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:

        """!!!"""
        # print("My_LlamaAttention_SDPA---Now_img_txt_token_spans:", Now_img_txt_token_spans)   # 成功传入！
        assert len(Now_img_txt_token_spans)==1, "Error from My_LlamaAttention_SDPA"
        
        # print("Attention_Change_Mod_4", Attention_Change_Mod)     # 成功传入！
        ...
        if past_key_value is not None:
            ...
            # print(inspect.getsource(past_key_value.update))          # 已查看源码，这里会更新past_key_value的值！！！
        ...
        # print(All_Now_img_txt_token_spans)
        # print(All_Now_img_txt_token_spans[0])
        # print(f"Q_K_V_states: {query_states.size()} | {key_states.size()} | {value_states.size()} |")
        # print()
        ...
        if Attention_Change_Mod == None:
            # print(f"Attention_Change_Mod = {Attention_Change_Mod}")
        ...
        elif Attention_Change_Mod == "No_change":
            # print(f"Attention_Change_Mod = {Attention_Change_Mod}")
            attn_output = torch.nn.functional.scaled_dot_product_attention(
                query_states,
                key_states,
        ...
        elif Attention_Change_Mod == "Enhance_img":
            # print(f"Attention_Change_Mod = {Attention_Change_Mod}")
            
            New_key_states = copy.deepcopy(key_states)
            
            for span in All_Now_img_txt_token_spans[0]:
                if span[0] == "image":
                    left_idx = span[1]
                    right_idx = span[2]
                    New_key_states[:, :, left_idx:right_idx, :] *= 1.2
        ...
        elif Attention_Change_Mod == "Enhance_text":
            # print(f"Attention_Change_Mod = {Attention_Change_Mod}")
            ...
            for span in All_Now_img_txt_token_spans[0]:
                if span[0] == "image":
                    left_idx = span[1]
                    right_idx = span[2]
                    New_key_states[:, :, left_idx:right_idx, :] *= 0.3
            ...
            raise ValueError("Attention_Change_Mod error!")
...
class My_LlamaDecoderLayer(LlamaDecoderLayer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # print("Attention_Change_Mod_3:", Attention_Change_Mod)    # 成功传入！
        ...
        hidden_states, self_attn_weights, present_key_value = self.self_attn(
            ...
            Attention_Change_Mod = Attention_Change_Mod,
            **kwargs,
        )
...
class My_LlamaModel(LlamaModel):
    ...
    def forward(
        ...
        # print("Attention_Change_Mod_2", Attention_Change_Mod)
        ...
        if attention_mask != None:
            ...
            print("attention_mask[0][0][-3]:", attention_mask[0][0][-3])                      ##################################
        """
...
class My_LlamaForCausalLM(LlamaForCausalLM):
    ...
    def forward(
        ...
        print("Attention_Change_Mod_1_1:", Attention_Change_Mod)
        ...
        # 错误：（注意，past_key_values这里要置为None，不然会沿用之前计算的 K,V,而这些 K,V是没屏蔽image tokens的。）
        # 修正：上述说法错误，由于这里缓存的是 K,V矩阵，还没计算attention呢，所以不会存在之前attention考虑了image tokens的问题
        print("Attention_Change_Mod_1_2:", Attention_Change_Mod)
        ...
        # Can predict same result without img -> image unrelated
        if Delta_Probs < 0.2:
            Attention_Change_Mod = "Enhance_img"
        else:
            Attention_Change_Mod = "No_change"
        

        """
        # method 2:
        Probs_1 = next_token_probs_1[0][next_token_id_1[0]]
        Probs_2 = next_token_probs_no_img[0][next_token_id_no_img[0]]

        Delta_Probs = Probs_1 - Probs_2

        if (Probs_1 < 0.5) and (Probs_2 < 0.5):
            Attention_Change_Mod = "Compare"
        elif (next_token_id_1[0] != next_token_id_no_img[0]) and (Delta_Probs < -0.3):
            Attention_Change_Mod = "Enhance_img"
        else:
            Attention_Change_Mod = "No_change"
        ...
        if Attention_Change_Mod == "Compare":
            print("Attention_Change_Mod_1_3:", Attention_Change_Mod)

            # ------------------------------------------------------------------------
            t_Attention_Change_Mod = "Enhance_img"
            outputs_Compare_1 = self.model(
                input_ids=input_ids,
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_values=past_key_values,
                ...
                return_dict=return_dict,
                Now_img_txt_token_spans = Now_img_txt_token_spans,
                Attention_Change_Mod = t_Attention_Change_Mod
            )
            
            hidden_states_Compare_1 = outputs_Compare_1[0]
            if self.config.pretraining_tp > 1:
                lm_head_slices_Compare_1 = self.lm_head.weight.split(self.vocab_size // self.config.pretraining_tp, dim=0)
                logits_Compare_1 = [F.linear(hidden_states_Compare_1, lm_head_slices_Compare_1[i]) for i in range(self.config.pretraining_tp)]
                logits_Compare_1 = torch.cat(logits_Compare_1, dim=-1)
            else:
                logits_Compare_1 = self.lm_head(hidden_states_Compare_1)
            logits_Compare_1 = logits_Compare_1.float()

            
            next_token_logit_Compare_1 = logits_Compare_1[:, -1, :]
    
            next_token_id_Compare_1 = torch.argmax(next_token_logit_Compare_1, dim=-1)  
    
            next_token_probs_Compare_1 = torch.nn.functional.softmax(next_token_logit_Compare_1, dim=-1)
    
            probs_value_Compare_1 = next_token_probs_Compare_1[0][next_token_id_Compare_1[0]]

            
            # ------------------------------------------------------------------------
            t_Attention_Change_Mod = "Enhance_text"
            outputs_Compare_2 = self.model(
                input_ids=input_ids,
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_values=past_key_values,
            ...
            hidden_states_Compare_2 = outputs_Compare_2[0]
            if self.config.pretraining_tp > 1:
                lm_head_slices_Compare_2 = self.lm_head.weight.split(self.vocab_size // self.config.pretraining_tp, dim=0)
                logits_Compare_2 = [F.linear(hidden_states_Compare_2, lm_head_slices_Compare_2[i]) for i in range(self.config.pretraining_tp)]
                logits_Compare_2 = torch.cat(logits_Compare_2, dim=-1)
            else:
                logits_Compare_2 = self.lm_head(hidden_states_Compare_2)
            logits_Compare_2 = logits_Compare_2.float()


    
            next_token_logit_Compare_2 = logits_Compare_2[:, -1, :]
    
            next_token_id_Compare_2 = torch.argmax(next_token_logit_Compare_2, dim=-1)  
    
            next_token_probs_Compare_2 = torch.nn.functional.softmax(next_token_logit_Compare_2, dim=-1)
    
            probs_value_Compare_2 = next_token_probs_Compare_2[0][next_token_id_Compare_2[0]]

            
            if (probs_value_Compare_1 >= probs_value_Compare_2) and (probs_value_Compare_1 > Probs_1):
                outputs = outputs_Compare_1
                
            elif (probs_value_Compare_2 > probs_value_Compare_1) and (probs_value_Compare_2 > Probs_1):
                outputs = outputs_Compare_2
        ...
        next_token_1 = My_tokenizer.decode(next_token_id_1)
        next_token_no_img = My_tokenizer.decode(next_token_id_no_img)
        next_token = My_tokenizer.decode(next_token_id)
        
        

        print("---------------------------------------------")

        print(f"[Has img]next_token_id: {next_token_id_1} -> {next_token_1} | logit_value: {logit_value_1} | probs_value: {probs_value_1}")
        print(f"[No  img]next_token_id: {next_token_id_no_img} -> {next_token_no_img} | logit_value: {logit_value_no_img} | probs_value: {probs_value_no_img}")
        print(f"[Fin img]next_token_id: {next_token_id} -> {next_token} | logit_value: {logit_value} | probs_value: {probs_value}")
        ...
        if Attention_Change_Mod == "Compare":
            next_token_Compare_1 = My_tokenizer.decode(next_token_id_Compare_1)
            next_token_Compare_2 = My_tokenizer.decode(next_token_id_Compare_2)
            
            print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
            print(f"[Compare 1]next_token_id: {next_token_id_Compare_1} -> {next_token_Compare_1} | probs_value: {probs_value_Compare_1}")
            print(f"[Compare 2]next_token_id: {next_token_id_Compare_2} -> {next_token_Compare_2} | probs_value: {probs_value_Compare_2}")
            
            if (probs_value_Compare_1 >= probs_value_Compare_2) and (probs_value_Compare_1 > Probs_1):
                print(f"Choose outputs_Compare_1")
            elif (probs_value_Compare_2 > probs_value_Compare_1) and (probs_value_Compare_2 > Probs_1):
                print(f"Choose outputs_Compare_2")
            else:
                print(f"Choose outputs_1")
            print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
        ...
        with open(output_file, "a") as f:
            ...
            f.write(f"[Has img]next_token_id: {next_token_id_1} -> {next_token_1} | logit_value: {logit_value_1} | probs_value: {probs_value_1}\n")
            f.write(f"[No  img]next_token_id: {next_token_id_no_img} -> {next_token_no_img} | logit_value: {logit_value_no_img} | probs_value: {probs_value_no_img}\n")
            f.write(f"[Fin img]next_token_id: {next_token_id} -> {next_token} | logit_value: {logit_value} | probs_value: {probs_value}\n")
    
    
            f.write("---------------------------------------------\n")
    
            f.write(f"[Delta_Probs]: {Delta_Probs} | Probs_1: {Probs_1} | Probs_2: {Probs_2} |\n")
            ...
            if Attention_Change_Mod == "Compare":
                ...
                f.write("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n")
                f.write(f"[Compare 1]next_token_id: {next_token_id_Compare_1} -> {next_token_Compare_1} | probs_value: {probs_value_Compare_1}\n")
                f.write(f"[Compare 2]next_token_id: {next_token_id_Compare_2} -> {next_token_Compare_2} | probs_value: {probs_value_Compare_2}\n")
                
                if (probs_value_Compare_1 >= probs_value_Compare_2) and (probs_value_Compare_1 > Probs_1):
                    f.write(f"Choose outputs_Compare_1\n")
                elif (probs_value_Compare_2 > probs_value_Compare_1) and (probs_value_Compare_2 > Probs_1):
                    f.write(f"Choose outputs_Compare_2\n")
                else:
                    f.write(f"Choose outputs_1\n")
                f.write("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n\n")
...
class LlavaLlamaForCausalLM(My_LlamaForCausalLM, LlavaMetaForCausalLM):
    ...
    def forward(
        ...
        return super().forward(
            ...
            My_tokenizer = self.My_tokenizer
        )
